...
## 2026-08-12 - 小程序本地 SWR 缓存优化

### 任务目标

优化微信小程序每次进入首页和常用页面时的体感加载速度。第一阶段不改 HTTP 协议、不引入 ETag / 304，只做客户端本地缓存优先展示和后台静默刷新，保持“缓存负责快，服务器负责真”。

### 具体修改内容

- 新增 `miniprogram/utils/cache.js`，统一管理页面级本地缓存，支持 TTL、过期元信息、环境隔离 key、身份短 hash key、stale fallback、前缀清理和公共活动 payload 清洗。
- 小程序首页接入 `/api/public/bootstrap` 本地缓存：有缓存时立即渲染近期活动和通知配置，后台静默请求最新数据；下拉刷新强制请求。
- 小程序活动列表接入第一页公开列表缓存，活动系列使用 1 小时公共缓存，分页加载仍实时请求。
- 活动详情缓存公共活动 base 和小程序配置；报名状态、感兴趣状态、订阅状态和权限状态仍以当前设备 / 服务端校准结果为准。
- 「我的」、我的报名、我的反馈和身份网络接入身份隔离缓存，缓存 key 使用当前匿名设备身份短 hash。
- 报名、取消报名、感兴趣、提交反馈、编辑公开资料、提交活动和身份同步后主动清理相关个人缓存与公共活动缓存。
- README、CHANGELOG 和小程序说明同步补充缓存策略与禁止缓存数据边界。

### 设计决策原因

- 小程序端本地缓存能绕过网络往返，优先改善用户感知速度；服务端 Redis 仍适合后续降低后端查询压力。
- 公共活动缓存写入前清理 `permissions`、`myRegistration`、`interestedByMe` 等 viewer-specific 字段，避免不同身份之间串状态。
- 个人数据只按当前匿名身份短 hash 缓存，避免多个设备或身份网络切换后互相读取旧数据。
