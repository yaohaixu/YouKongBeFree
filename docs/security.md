...
- 编辑已有活动时会申请短期编辑软锁并自动续期，常规主发起人和共同发起人提交时必须带 `editLockToken`；同时服务端校验 `activityVersion`，防止两个人同时提交导致静默覆盖。管理员保留无锁兜底编辑能力。
...
- 公开资料编辑已拆为独立 `profile-editor.html`，公开工作台只展示头像、昵称和简介摘要；公开资料 API 仍只返回最小公开字段，不暴露完整匿名身份、后台手机号、管理 token 或 Community Trust 分数。
- 富文本正文图片需先在浏览器或小程序端压缩后上传，原图最大 10MB，服务端会校验图片内容魔数并只接受压缩后 10MB 以内的图片，文件存储到本地 `uploads/` 或 CloudBase Storage；CloudBase 正文图片通过 `/api/files?fileId=...` 代理生成最新临时地址，避免公开页保存过期临时 URL。
...
- 社区举报会记录 `identityNetworkId` 并按身份网络优先去重；同一个已同步身份网络内换设备举报同一活动只会返回已有记录。
- 报名写入使用活动维度串行锁、匿名身份维度幂等报名 ID 和确认 token，降低重复提交、隐私数据沉淀和同一活动并发超员风险。
- 报名、感兴趣、活动反馈和“我的”相关查询会按身份网络优先聚合；设备合并后，同一身份网络对同一活动仍只算一次报名、一次感兴趣和一次匿名反馈。
...
- 设备同步邀请目前是链接 / 二维码制，任何拿到链接且未过期的人都可以把当前设备合并进目标身份网络；后续可增加发起端二次确认、邀请撤销列表、设备重命名、登录通知和更细的合并审计页。
- Identity Network 已预留外部凭证集合，但微信小程序绑定、凭证撤销、平台账号解绑和多平台冲突合并尚未实现。
- 共同发起邀请目前是链接制，任何拿到链接且未过期的人都可以接受；如果活动协作涉及更高风险操作，建议后续支持指定 Community ID / 手机号、邀请备注、邀请撤销和邀请审计页。
