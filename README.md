...
状态：`0.10.0` 已完成开发、本地验证与 CloudBase 线上部署。本版本重点加固报名名额保护、操作日志隐私和运营归档能力。
...
- GitHub 状态：`0.10.0` 按双分支流程维护，推送到 `dev` / `main` 后 GitHub Actions 会自动运行测试与构建；最新提交请以 `git log --oneline --decorate --graph --all` 为准。
