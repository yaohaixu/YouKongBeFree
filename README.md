...
状态：`0.16.4` 优化首页近期活动和移动端活动操作按钮：PC 首页改为横向活动行，移动端后台 / 我的活动按钮统一为稳定两列尺寸。
...
- 独立活动列表页：首页最多读取 3 条近期活动；PC 首页展示为最多 2 条横向活动行，移动端保持横向滑动活动卡；`activities.html` 展示所有近期活动，`activities.html?view=history` 展示历史活动。
- 移动端活动操作按钮：全部活动管理和我的活动列表在手机端使用统一两列按钮网格，避免 3 个以上操作按钮被拉成长条。
