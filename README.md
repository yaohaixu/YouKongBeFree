...
状态：`0.17.1` 完成首页近期活动 PC 端视觉小修：横向活动长条的海报与右侧文字之间恢复独立列间距，并升级静态资源参数到 `v=0.17.1`，避免线上旧 CSS 缓存影响展示。
...
- 独立活动列表页：首页最多读取 3 条近期活动；PC 首页使用与近期活动详情页一致的完整宽度横向活动长条，海报与文字之间保留独立呼吸间距，移动端保持横向滑动活动卡；`activities.html` 展示所有近期活动，`activities.html?view=history` 展示历史活动。
