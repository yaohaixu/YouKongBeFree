...
状态：`0.4.1` 已完成开发、本地验证与 CloudBase 线上部署。本版本优化审核待办、报名状态限制、保存/删除反馈、页面留白、移动端表单布局和整体动效视觉。
...
- 视觉体验：1200px 左右内容最大宽度、自适应左右留白、移动端单列布局、卡片 hover 和动态内容进入动效。
...
- CloudBase `0.4.1` 线上部署通过：静态页已引用 `styles.css?v=0.4.1`、`script.js?v=0.4.1`、`app.js?v=0.4.1`，线上 CSS 可查到本次新增的 `--page-gutter`、`review-cover`、`toast` 和 `dynamic-reveal` 样式。
- 线上冒烟产生的测试成员、活动和报名记录已清理。
- GitHub 状态：`0.4.1` 按双分支流程维护，最新提交请以 `git log --oneline --decorate --graph --all` 为准。
