...
- 所有页面级本地缓存统一走 `utils/cache.js`，缓存 key 包含 `yk:mp`、运行环境、业务语义和版本，避免测试版与正式版串数据。
- 首页、活动列表、活动详情基础信息采用 SWR：有缓存时立即渲染，后台静默请求最新数据；没有缓存时才展示正常 loading。
- 活动详情只缓存公共 base；是否报名、是否感兴趣、是否订阅、当前权限等状态仍以当前设备和服务端校准结果为准。
- 我的页面、我的报名、我的反馈和身份网络缓存会带当前匿名身份短 hash，避免不同设备 / 身份互相读取缓存。
- 报名、取消报名、感兴趣、提交反馈、保存公开资料、提交活动和身份同步后，会主动清理相关缓存。
- 管理员权限、审核状态、AI 配置、API Key、token 和敏感身份信息不进入普通小程序缓存。
