...
const { buildActivityMessages, buildFeedbackMessages, buildRoomLogMessages, getActivePrompt } = require("./prompts/service");
...
const AI_SCENES = ["activity", "feedback", "room_log", "report", "manual"];
...
async function analyzeRoomLog(store, input = {}, context = {}) {
  const settings = await getAiSettings(store);
  if (!settings.enabled && !context.forceAi) {
    return { skipped: true, reason: "disabled", decision: { call: false, reason: "disabled" }, report: null };
  }
  const payload = roomLogPayload(input);
  const prompt = await getActivePrompt(store, "room_log", promptVersionFor(settings, "room_log"));
  return runAiAnalysis(store, {
    settings,
    scene: "room_log",
    contentType: "room_log",
    activityId: context.roomLogId || context.activityId || "",
    payload,
    prompt,
    messages: buildRoomLogMessages(prompt, payload, FEEDBACK_SCHEMA_TEXT),
    normalizer: normalizeFeedbackReport,
    decision: { call: true, reason: "room-log" },
    context,
  });
}
