...
async function analyzeActivity(store, input = {}, context = {}) {
  ...
  if (cached) {
    ...
    return { skipped: false, reason: decision.reason, decision, report: normalizeAnalysisReport(cached), cacheHit: true };
  }
  ...
  try {
    ...
    return { skipped: false, reason: decision.reason, decision, report, raw: result.raw };
  } catch (error) {
    await logAiUsage(store, {
      provider: settings.provider,
      model: settings.model,
