...
async function analyzeActivity(store, input = {}, context = {}) {
  ...
  const effectiveSettings = {
    ...settings,
    promptVersion: prompt.version || promptVersionFor(settings, "activity"),
  };
  const key = cacheKey(payload, effectiveSettings);
  ...
  if (cached) {
    await logAiUsage(store, {
      ...
      cacheHit: true,
      promptVersion: effectiveSettings.promptVersion,
    });
    return { skipped: false, reason: decision.reason, decision, report: normalizeAnalysisReport(cached), cacheHit: true, prompt };
  }
  ...
  try {
    ...
    return { skipped: false, reason: decision.reason, decision, report, raw: result.raw, prompt };
  } catch (error) {
    await logAiUsage(store, {
      provider: settings.provider,
      model: settings.model,
...
async function analyzeFeedback(store, input = {}, context = {}) {
  ...
  const effectiveSettings = {
    ...settings,
    promptVersion: prompt.version || promptVersionFor(settings, "feedback"),
  };
  const key = cacheKey({ type: "feedback", ...payload }, effectiveSettings);
  const bypassCache = Boolean(context.forceAi || context.bypassCache);
  const cached = bypassCache ? null : await getCachedAnalysis(store, key);
  if (cached) {
    await logAiUsage(store, {
    ...
    return { skipped: false, reason: "feedback", decision: { call: true, reason: "feedback" }, report: normalizeFeedbackReport(cached), cacheHit: true, prompt };
  }
  ...
  const messages = buildFeedbackMessages(prompt, payload, FEEDBACK_SCHEMA_TEXT);
  const adapter = getProviderAdapter(settings.provider);
  try {
    const result = await withRetry(
      () => adapter.chatCompletion({ ...settings, apiKey }, messages),
    ...
    return { skipped: false, reason: "feedback", decision: { call: true, reason: "feedback" }, report, raw: result.raw, prompt };
  } catch (error) {
    await logAiUsage(store, {
      provider: settings.provider,
      model: settings.model,
