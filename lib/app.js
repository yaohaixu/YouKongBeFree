...
function sortActivities(activities, sort = "created-desc") {
...
function createApp(options = {}) {
  ...
  app.get("/api/users", requireAdmin, asyncRoute(async (req, res) => {
    ...
    const users = (await store.all("users"))
      .map((user) => publicUser(user, { includePhone: true }))
      .filter((user) => {
        const roles = user.roles || [user.role || "member"];
        return (!role || roles.includes(role))
          && includesKeyword([user.nickname, user.phone, roles.join(" ")], keyword);
      })
      .sort((a, b) => (a.id === "admin" ? -1 : b.id === "admin" ? 1 : a.nickname.localeCompare(b.nickname, "zh-CN")));
    const { data, pageInfo } = paginate(users, req.query);
    res.json({ users: data, pageInfo });
  }));
  ...
  app.get("/api/modules", asyncRoute(async (req, res) => {
    const keyword = cleanText(req.query.q).toLowerCase();
    const modules = (await store.all("modules"))
      .filter((module) => includesKeyword([module.name, module.description], keyword))
      .sort((a, b) => (a.createdAt || "").localeCompare(b.createdAt || ""));
    if (req.query.paged === "true") {
      const { data, pageInfo } = paginate(modules, req.query);
  ...
  app.get("/api/activities", asyncRoute(async (req, res) => {
    ...
    const payload = await toActivityListPayload(activities);
    const keyword = cleanText(req.query.q).toLowerCase();
    const status = cleanText(req.query.status);
    const moduleId = cleanText(req.query.moduleId);
    const from = req.query.from ? new Date(String(req.query.from)).getTime() : 0;
    const to = req.query.to ? new Date(`${String(req.query.to)}T23:59:59`).getTime() : 0;
    const filtered = payload.filter((activity) => {
    ...
    const sorted = sortActivities(filtered, req.query.sort || (owner || pending || all ? "created-desc" : "start-asc"));
    const { data, pageInfo } = paginate(sorted, req.query);
    res.json({ activities: data, pageInfo });
  }));
  ...
  app.get("/api/logs", requireAdmin, asyncRoute(async (req, res) => {
    const keyword = cleanText(req.query.q).toLowerCase();
    const logs = (await store.all("logs"))
      .filter((log) => includesKeyword([
        log.action,
        log.actionLabel,
        log.actorName,
        log.actorPhone,
    ...
    const { data, pageInfo } = paginate(logs, req.query);
