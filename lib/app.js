...
async function interestMapForActivities(activityIds = [], req = null) {
  ...
  if (!ids.length) return new Map();
  const identity = req ? requestIdentity(req) : null;
  const [{ data: interests }, myInterests] = await Promise.all([
    ...
    identity?.id
      ? store.query("activityInterests", {
        page: 1,
        pageSize: ids.length,
        maxPageSize: Math.max(ids.length, 1),
        filters: [
          { field: "activityId", op: "in", value: ids },
          { field: "identityId", op: "eq", value: identity.id },
        ],
      })
...
async function countOwnedActivitiesForRequest(req, user = null, filters = [], options = {}) {
  ...
  if (!ownerFilterSets.length) return 0;
  if (ownerFilterSets.length === 1) {
    return countRecords("activities", [...filters, ...ownerFilterSets[0]], options);
  }

  const ownerCounts = await Promise.all(ownerFilterSets.map((ownerFilters) =>
    countRecords("activities", [...filters, ...ownerFilters], options)));
  const intersectionFilters = [
    ...filters,
    ...ownerFilterSets.flatMap((ownerFilters) => ownerFilters),
  ];
  const duplicated = await countRecords("activities", intersectionFilters, options);
  return ownerCounts.reduce((sum, value) => sum + Number(value || 0), 0) - Number(duplicated || 0);
}
...
async function ownedActivityPageForRequest(req, user = null, options = {}) {
  ...
  if (!ownerFilterSets.length) return { data: [], pageInfo: activityPageInfo(0, page, pageSize, 0) };
  ...
  const duplicated = await countRecords("activities", [
    ...filters,
    ...ownerFilterSets.flatMap((ownerFilters) => ownerFilters),
  ], { keyword, keywordFields });
  const total = results.reduce((sum, result) => sum + Number(result.pageInfo?.total || 0), 0) - Number(duplicated || 0);
...
async function publicProfileActivities(identityId = "", req = null) {
  const { data } = await store.query("activities", {
    page: 1,
    pageSize: 24,
    maxPageSize: 24,
    filters: [
  ...
  const visible = data.filter((activity) => !activity.isHidden);
...
async function toActivityPayload(activity, options = {}) {
  ...
  const [module, owner, collaborator, friend, initiatorProfile, activityRegistrations, interestMap] = await Promise.all([
    ...
    activity.anonymousIdentityId ? identityProfileById(activity.anonymousIdentityId, { fallbackName: activity.initiator }) : null,
...
async function toActivityListPayload(activities, options = {}) {
  ...
  const [moduleMap, userMap, friendMap, profileMap, interestMap] = await Promise.all([
    ...
    identityProfileMap(normalizedActivities.map((activity) => activity.anonymousIdentityId)),
  ...
  return Promise.all(normalizedActivities.map((activity) => {
    ...
    const initiatorProfile = profileMap.get(activity.anonymousIdentityId) || null;
    const capacity = effectiveCapacity(activity);
...
function canManageActivity(activity, user, req) {
  ...
  if (user && (userCan(user, "activities", "view") || activity.createdBy === user.id)) return true;
...
function createApp(options = {}) {
  ...
  app.get("/api/profile/me", asyncRoute(async (req, res) => {
    const identity = requestIdentity(req);
    const profile = await identityProfileById(identity.id);
    res.json({
      profile: profile || await publicIdentityProfile(null, { identityId: identity.id }),
      anonymous: publicIdentity(identity),
    });
  }));

  app.put("/api/profile/me", profileAvatarUpload.single("avatar"), asyncRoute(async (req, res) => {
    const identity = requestIdentity(req);
    ...
    const existing = await store.findById("identityProfiles", identity.id);
    ...
    const patch = {
      id: identity.id,
      communityId: communityId(identity.id),
    ...
    let saved;
    if (existing) {
      saved = await store.update("identityProfiles", identity.id, patch);
    } else {
      saved = {
        ...patch,
        createdAt: now,
    ...
    await writeLog(req, "profile.update", {
      ...
      targetName: input.displayName || communityId(identity.id),
      detail: "保存我的公开资料",
    });
    res.json({ profile: await publicIdentityProfile(saved, { identityId: identity.id }) });
  }));
  ...
  app.get("/api/activities", asyncRoute(async (req, res) => {
    ...
    if (adminPendingMode) {
      ...
      res.json({
        activities: await toActivityListPayload(visible, { req }),
        pageInfo: {
          page,
          pageSize,
          total,
    ...
    res.json({ activities: await toActivityListPayload(visibleActivities, { req }), pageInfo });
  }));
  ...
  app.get("/api/activities/:id", asyncRoute(async (req, res) => {
    ...
    res.json({ activity: await toActivityPayload(activity, { req }) });
  }));
  ...
  app.get("/api/my/registrations", asyncRoute(async (req, res) => {
    const identity = requestIdentity(req);
    const { data, pageInfo } = await store.query("registrations", {
      ...pageQueryOptions(req.query),
      filters: [{ field: "identityId", op: "eq", value: identity.id }],
      sort: [{ field: "createdAt", direction: "desc" }],
    });
  ...
  app.post("/api/activities/:id/feedbacks", registrationRateLimiter, asyncRoute(async (req, res) => {
    ...
    const identity = requestIdentity(req);
    const feedbackId = makeActivityFeedbackId(activity.id, identity.id);
    const existing = await store.findById("activityFeedbacks", feedbackId);
    if (existing) {
  ...
  app.post("/api/activities/:id/cancel", requirePermission("activities", "cancel"), activityMutationRateLimiter, asyncRoute(async (req, res) => {
    const updated = await withMutationLock(`activity:${req.params.id}`, async () => {
      const activity = normalizeActivity(await store.findById("activities", req.params.id));
      if (!activity) {
        throw Object.assign(new Error("找不到该活动"), { statusCode: 404 });
  ...
  app.post("/api/activities/:id/end", requirePermission("activities", "end"), activityMutationRateLimiter, asyncRoute(async (req, res) => {
    const updated = await withMutationLock(`activity:${req.params.id}`, async () => {
      const activity = normalizeActivity(await store.findById("activities", req.params.id));
      if (!activity) {
        throw Object.assign(new Error("找不到该活动"), { statusCode: 404 });
  ...
  app.post("/api/activities/:id/register", registrationRateLimiter, asyncRoute(async (req, res) => {
    ...
    const result = await withMutationLock(`registration:${req.params.id}`, async () => {
      ...
      const registrationId = makeRegistrationId(activity.id, `identity:${identity.id}`);
      const existing = await findExistingRegistration(activity.id, { id: registrationId, identityId: identity.id });
  ...
  app.post("/api/activities/:id/interests", registrationRateLimiter, asyncRoute(async (req, res) => {
    const identity = requestIdentity(req);
    const result = await withMutationLock(`interest:${req.params.id}:${identity.id}`, async () => {
      const activity = normalizeActivity(await store.findById("activities", req.params.id));
      if (!activity) {
        throw Object.assign(new Error("找不到该活动"), { statusCode: 404 });
      }
      ...
      const now = new Date().toISOString();
      const interest = {
        id: makeActivityInterestId(activity.id, identity.id),
        activityId: activity.id,
        identityId: identity.id,
        identitySnapshot: publicIdentity(identity),
        createdAt: now,
