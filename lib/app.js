...
function summarizeActivityCounts(byStatus = {}) {
  ...
  return {
    total,
    byStatus,
    reviewing: Number(byStatus[ACTIVITY_STATUS.ADMIN_REVIEW] || 0)
...
function createApp(options = {}) {
  ...
  app.get("/api/trust-profiles/:id", requireAdmin, asyncRoute(async (req, res) => {
    ...
    const [{ data: events }, { data: activities }] = await Promise.all([
      store.query("trustEvents", {
        ...
        pageSize: 100,
        maxPageSize: 100,
        filters: [{ field: "identityId", op: "eq", value: profile.id }],
  ...
  app.get("/api/activities", asyncRoute(async (req, res) => {
    ...
    const defaultSort = owner || pending || all
      ...
      : publicView === "history"
        ...
        : "start-asc";
    if (status === "reviewing") {
      filters.push({ field: "status", op: "in", value: [ACTIVITY_STATUS.ADMIN_REVIEW, ACTIVITY_STATUS.COLLABORATOR_REVIEW] });
  ...
  app.post("/api/activities/:id/reanalyze", requireAdmin, asyncRoute(async (req, res) => {
    ...
    const updated = await store.update("activities", activity.id, {
      ...activityRiskPatch(analysisReport, { identity, trustProfile }),
      updatedAt: new Date().toISOString(),
    });
  ...
  app.post("/api/activities/:id/reports", asyncRoute(async (req, res) => {
    ...
    const safetyConfig = submitted.context?.safetyConfig || await getSafetyConfig(store);
    if (!submitted.existing && reportCount >= Number(safetyConfig.report?.threshold || 3)) {
      const trustProfile = activity.anonymousIdentityId ? await store.findById("trustProfiles", activity.anonymousIdentityId) : null;
      const analysisContext = {
        ...submitted.context,
        identity: { ...(submitted.context?.identity || {}), id: activity.anonymousIdentityId || submitted.context?.identity?.id || "" },
        trustProfile,
      };
      const analysis = await analyzeActivitySafety(store, activity, {
        ...analysisContext,
      ...
      const riskPatch = activityRiskPatch(analysisReport, analysisContext);
      const shouldHide = safetyConfig.report?.hideOnHighRisk === true
        && Number(riskPatch.riskScore || 0) >= Number(safetyConfig.report?.autoHideThreshold || 90);
      updated = await store.update("activities", activity.id, {
        ...riskPatch,
        reportCount,
        isHidden: shouldHide ? true : activity.isHidden || false,
        updatedAt: new Date().toISOString(),
      }) || updated;
      if (trustProfile && Number(riskPatch.riskScore || 0) >= Number(safetyConfig.report?.highRiskThreshold || 70)) {
        await recordCommunityEvent(store, trustProfile, {
          type: "community.report.confirmed",
          source: "report",
          reason: `社区举报达到阈值并触发高风险分析：${activity.title}`,
          reportConfirmedIncrement: 1,
          activityId: activity.id,
          payload: { riskScore: riskPatch.riskScore, confidenceScore: riskPatch.confidenceScore, reportCount },
        }, safetyConfig.trust);
      }
    }
  ...
  app.post("/api/activities", activityMutationRateLimiter, upload.single("cover"), asyncRoute(async (req, res) => {
    ...
    const prepared = await prepareActivitySubmission(store, req, input, { intent, activityId });
    if (!prepared.ok) {
      await removeUploadedFile(req.file);
    ...
    const activity = {
      ...
      coverFileId,
      registrationCount: 0,
      status: prepared.analysis.policy.status,
      reviewStep: prepared.analysis.policy.reviewStep,
      reviewLogs: [],
      ...
      createdAt: now,
      updatedAt: now,
      publishedAt: prepared.analysis.policy.status === ACTIVITY_STATUS.PUBLISHED ? now : "",
    };
    const analysisReport = await storeAnalysisReport(store, activity.id, prepared.analysis, prepared.context);
    Object.assign(activity, activityRiskPatch(analysisReport, prepared.context));
    await store.insert("activities", activity);
    if (!asDraft && activity.status === ACTIVITY_STATUS.PUBLISHED) {
      await emitActivityPublishedEvent(activity, prepared.context.trustProfile, `活动直接发布：${activity.title}`);
    }
    ...
    await writeLog(req, submitAction, {
      ...
      detail: asDraft
        ...
        : `发起活动：${activity.title}（策略：${activity.policyAction || "publish"}，风险分 ${activity.riskScore}）`,
    });
    if (!asDraft && activity.status === ACTIVITY_STATUS.ADMIN_REVIEW) {
      await writeSystemLog("activity.risk_review", {
        targetType: "activity",
        targetId: activity.id,
        targetName: activity.title,
        detail: `活动进入社区复核：${activity.title}（风险分 ${activity.riskScore}）`,
      });
    }
    res.json({
      activity: await toActivityPayload(activity),
      manageToken,
      policy: prepared.analysis.policy,
    });
  }));
  ...
  app.put("/api/activities/:id", activityMutationRateLimiter, upload.single("cover"), asyncRoute(async (req, res) => {
    ...
    const prepared = await prepareActivitySubmission(store, req, input, { intent, activityId: activity.id });
    if (!prepared.ok) {
      await removeUploadedFile(req.file);
      res.status(prepared.statusCode || 400).json({ error: prepared.error || "活动暂时不能发布，请稍后再试" });
      return;
    ...
    const updated = await store.update("activities", activity.id, {
      ...
      status: prepared.analysis.policy.status,
      reviewStep: prepared.analysis.policy.reviewStep,
      publishedAt: prepared.analysis.policy.status === ACTIVITY_STATUS.PUBLISHED ? (activity.publishedAt || new Date().toISOString()) : activity.publishedAt || "",
      updatedAt: new Date().toISOString(),
    });
    const analysisReport = await storeAnalysisReport(store, activity.id, prepared.analysis, prepared.context);
    const patched = await store.update("activities", activity.id, activityRiskPatch(analysisReport, prepared.context));
    const finalUpdated = patched || updated;
    if (!asDraft && finalUpdated.status === ACTIVITY_STATUS.PUBLISHED && ![ACTIVITY_STATUS.PUBLISHED, ACTIVITY_STATUS.FULL].includes(activity.status)) {
      await emitActivityPublishedEvent(finalUpdated, prepared.context.trustProfile, `活动重新提交后发布：${finalUpdated.title}`);
    }
    await writeLog(req, asDraft ? "activity.update_draft" : "activity.update_submit", {
      ...
      detail: asDraft
        ...
        : `重新发起活动：${finalUpdated.title}（策略：${finalUpdated.policyAction || "publish"}，风险分 ${finalUpdated.riskScore}）`,
    });
    res.json({ activity: await toActivityPayload(finalUpdated), policy: prepared.analysis.policy });
  }));
  ...
  app.post("/api/activities/:id/review", requireLogin, activityMutationRateLimiter, asyncRoute(async (req, res) => {
    try {
      ...
      if (updated.status === ACTIVITY_STATUS.PUBLISHED && updated.anonymousIdentityId) {
