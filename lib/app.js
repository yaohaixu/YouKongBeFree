    ...
    const result = await withMutationLock(`identity-sync:${hashIdentitySyncInviteToken(token)}`, async () => {
      ...
      const sourceDevices = sourceNetwork
        ...
        : [{
          ...
          label: normalizeDeviceLabel(req.body?.label, deviceLabelFromIdentity(sourceContext.identity)),
      ...
      const profileChoice = cleanText(req.body?.profileSource || "target");
      const selectedProfile = selectProfileForMerge(targetProfile, sourceProfile, profileChoice, req.body?.profile || {});
      const profilePatch = {
        id: targetNetwork.id,
