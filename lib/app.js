...
const ACTIVITY_EDIT_LOCK_TTL_MS = 5 * 60 * 1000;
...
async function coInitiatorProfilesForActivity(activity = {}, profileMap = null) {
  ...
  const ids = records.length
    ? records.map((item) => item.identityId).filter(Boolean)
...
async function coInitiatedActivityIdsForRequest(req) {
  ...
  return Array.from(new Set(data.map((item) => item.activityId).filter(Boolean)));
}
...
async function toActivityPayload(activity, options = {}) {
  ...
  const [module, owner, collaborator, friend, initiatorProfile, coProfileMap, activityRegistrations, interestMap] = await Promise.all([
...
async function toActivityListPayload(activities, options = {}) {
  ...
  const identityIds = normalizedActivities.flatMap((activity) => [activity.anonymousIdentityId, ...coInitiatorIdentityIds(activity)]);
...
function isMainActivityInitiator(activity, user, req) {
  ...
  const identityId = requestIdentityId(req);
  if (identityId && activity.anonymousIdentityId === identityId) return true;
  return hasActivityManageToken(activity, req);
}

function isActivityCoInitiator(activity, req) {
  const identityId = requestIdentityId(req);
  return Boolean(identityId && coInitiatorIdentityIds(activity).includes(identityId));
}
...
function createApp(options = {}) {
  ...
  app.get("/api/co-initiator-invites/:token", asyncRoute(async (req, res) => {
    ...
    const inviterProfile = invite.createdByIdentityId
      ? await identityProfileById(invite.createdByIdentityId, { fallbackName: activity.initiator })
      : null;
    const myProfile = await identityProfileById(requestIdentityId(req));
    res.json({
      invite: {
        id: invite.id,
  ...
  app.post("/api/co-initiator-invites/:token/accept", activityMutationRateLimiter, asyncRoute(async (req, res) => {
    ...
    const tokenHash = hashCoInitiatorInviteToken(token);
    const identity = requestIdentity(req);
    const profileRecord = await store.findById("identityProfiles", identity.id);
    ...
    const updated = await withMutationLock(`co-invite:${tokenHash}`, async () => {
      ...
      if (activity.anonymousIdentityId === identity.id) {
        throw Object.assign(new Error("主发起人不需要接受自己的共同发起人邀请"), { statusCode: 400 });
      }
      const now = new Date().toISOString();
      const coRecord = {
        id: makeCoInitiatorId(activity.id, identity.id),
        activityId: activity.id,
        identityId: identity.id,
        role: invite.role || CO_INITIATOR_ROLE,
        status: "active",
      ...
      const existingCoRecord = await store.findById("activityCoInitiators", coRecord.id);
      if (existingCoRecord) {
      ...
      const ids = Array.from(new Set([...coInitiatorIdentityIds(activity), identity.id]));
      const savedActivity = await store.update("activities", activity.id, {
        coInitiatorIdentityIds: ids,
        activityVersion: normalizeActivityVersion(activity) + 1,
        updatedAt: now,
      }) || { ...activity, coInitiatorIdentityIds: ids };
  ...
  app.post("/api/activities/:id/co-initiator-invites", activityMutationRateLimiter, asyncRoute(async (req, res) => {
    ...
    const invite = {
      ...
      role: CO_INITIATOR_ROLE,
      status: "pending",
      createdByIdentityId: requestIdentityId(req),
  ...
  app.delete("/api/activities/:id/co-initiators/:identityId", activityMutationRateLimiter, asyncRoute(async (req, res) => {
    ...
    if (!coInitiatorIdentityIds(activity).includes(identityId)) {
      res.status(404).json({ error: "找不到该共同发起人" });
      return;
    }
    const now = new Date().toISOString();
    const record = await store.findById("activityCoInitiators", makeCoInitiatorId(activity.id, identityId));
    if (record) {
    ...
    const ids = coInitiatorIdentityIds(activity).filter((id) => id !== identityId);
    const updated = await store.update("activities", activity.id, {
      coInitiatorIdentityIds: ids,
      activityVersion: normalizeActivityVersion(activity) + 1,
      updatedAt: now,
  ...
  app.post("/api/activities", activityMutationRateLimiter, upload.single("cover"), asyncRoute(async (req, res) => {
    ...
    const prepared = asDraft
      ? { ok: true, context: { identity: requestIdentity(req) } }
