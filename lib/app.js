...
function createApp(options = {}) {
  ...
  app.get("/api/trust-profiles", requireAdmin, asyncRoute(async (req, res) => {
    ...
    res.json({ profiles, pageInfo });
  }));

  app.get("/api/trust-profiles/:id", requireAdmin, asyncRoute(async (req, res) => {
    const profile = await store.findById("trustProfiles", req.params.id);
    if (!profile) {
    ...
    res.json({ profile, events, activities: await toActivityListPayload(activities) });
  }));
  ...
  app.post("/api/activities/:id/reports", asyncRoute(async (req, res) => {
    ...
    if (!submitted.existing && reportCount >= Number(safetyConfig.report?.threshold || 3)) {
      ...
      if (trustProfile && Number(riskPatch.riskScore || 0) >= Number(safetyConfig.report?.highRiskThreshold || 70)) {
        await recordTrustEvent(store, trustProfile, {
          type: "community.report.confirmed",
          reason: `社区举报达到阈值并触发高风险分析：${activity.title}`,
          delta: Number(safetyConfig.trust?.deltas?.communityReportConfirmed ?? -10),
          reportConfirmedIncrement: 1,
          activityId: activity.id,
          metadata: { riskScore: riskPatch.riskScore, reportCount },
        }, safetyConfig.trust);
      }
    }
      ...
      if (updated.status === ACTIVITY_STATUS.PUBLISHED && updated.anonymousIdentityId) {
        ...
        if (profile) {
          await recordTrustEvent(store, profile, {
            ...
            reason: `活动通过社区复核并发布：${updated.title}`,
            delta: Number(safetyConfig.trust?.deltas?.activityPublished ?? 3),
