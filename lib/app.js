...
async function getActivityRegistrations(activityId) {
  return (await store.all("registrations")).filter((item) => item.activityId === activityId);
}
...
async function memberDashboardPayload(user) {
  const [byStatus, pending] = await Promise.all([
    activityStatusCounts([{ field: "createdBy", op: "eq", value: user.id }]),
    pendingPreviewForUser(user, 3),
  ]);
  return {
...
function createApp(options = {}) {
  ...
  app.get("/api/session", asyncRoute(async (req, res) => {
    res.json({ user: publicUser(await getCurrentUser(req), { includePhone: true }) });
  }));
  ...
  app.get("/api/dashboard/me", requireLogin, asyncRoute(async (req, res) => {
    await sweepExpiredActivities({ reason: "dashboard-me" });
    res.json(await memberDashboardPayload(req.currentUser));
  }));
  ...
  app.post("/api/uploads/rich-image", requireLogin, richImageUpload.single("image"), asyncRoute(async (req, res) => {
    if (!req.file) {
  ...
  app.delete("/api/users/:id", requireAdmin, asyncRoute(async (req, res) => {
    ...
    await writeLog(req, "user.delete", {
      ...
      detail: `删除成员：${user ? `${user.nickname}（${maskPhone(user.phone)}）` : req.params.id}`,
    });
    res.json({ ok: true });
  }));
  ...
  app.get("/api/templates", requireLogin, asyncRoute(async (req, res) => {
    const keyword = cleanText(req.query.q).toLowerCase();
  ...
  app.get("/api/templates/:id", requireLogin, asyncRoute(async (req, res) => {
    const template = await store.find("templates", (item) => item.id === req.params.id);
    if (!template) {
  ...
  app.post("/api/activities", requireLogin, activityMutationRateLimiter, upload.single("cover"), asyncRoute(async (req, res) => {
    const input = parseActivityInput(req.body, req.currentUser.nickname, req.currentUser.phone);
  ...
  app.put("/api/activities/:id", requireLogin, activityMutationRateLimiter, upload.single("cover"), asyncRoute(async (req, res) => {
    const activity = normalizeActivity(await store.find("activities", (item) => item.id === req.params.id));
    if (!activity) {
      res.status(404).json({ error: "找不到该活动" });
      return;
    ...
    const input = parseActivityInput(req.body, activity.initiator, req.currentUser.phone);
    const intent = req.body.intent === "draft" ? "draft" : "submit";
    const asDraft = intent === "draft";
  ...
  app.post("/api/activities/:id/withdraw", requireLogin, activityMutationRateLimiter, asyncRoute(async (req, res) => {
    const activity = normalizeActivity(await store.find("activities", (item) => item.id === req.params.id));
    if (!activity) {
      res.status(404).json({ error: "找不到该活动" });
      return;
  ...
  app.post("/api/activities/:id/register", registrationRateLimiter, asyncRoute(async (req, res) => {
    ...
    const result = await withMutationLock(`registration:${req.params.id}`, async () => {
      ...
      const existing = registrations.find((item) => item.id === registrationId || item.phone === phone || item.phoneHash === phoneHash);
      if (existing && PUBLIC_ACTIVITY_STATUSES.includes(activity.status)) {
        return { registration: existing, activity, existing: true };
      }
      ...
      return { registration, activity: updated || activity, existing: false };
    });
    res.json({ registration: result.registration, activity: await toActivityPayload(result.activity), existing: result.existing });
  }));
  ...
  app.delete("/api/activities/:id/registrations/:registrationId", requireLogin, asyncRoute(async (req, res) => {
    ...
    await withMutationLock(`registration:${activity.id}`, async () => {
      removedRegistration = await store.find(
        "registrations",
        (item) => item.id === req.params.registrationId && item.activityId === activity.id
      );
