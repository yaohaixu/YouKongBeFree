...
function roomStatusFromLogs(logs = [], nowLocal = shanghaiLocalDateTime()) {
  const visible = logs.filter((log) => log && !log.isHidden);
  if (!visible.length) return emptyRoomStatus();
  const opened = visible
    .filter((log) => log.openedAt && !log.closedAt)
    .sort((a, b) => String(b.openedAt || b.updatedAt || "").localeCompare(String(a.openedAt || a.updatedAt || "")))[0];
  const scheduled = visible
    .filter((log) => !log.openedAt && !log.closedAt)
    .sort((a, b) => String(a.scheduledOpenAt || a.createdAt || "").localeCompare(String(b.scheduledOpenAt || b.createdAt || "")))[0];
  const closed = visible
    .filter((log) => log.closedAt)
    .sort((a, b) => String(b.closedAt || b.updatedAt || "").localeCompare(String(a.closedAt || a.updatedAt || "")))[0];
  ...
  const payload = publicRoomLog({ ...currentLog, status }, { includePrivate: false });
  const note = payload.openNote || payload.nightNote || "";
  return {
    status,
    tone: roomLogStatusTone(status),
    statusLabel: roomLogStatusLabel(status),
    title: status === ROOM_LOG_STATUS.OPENED
    ...
    text: note || (status === ROOM_LOG_STATUS.SCHEDULED && currentLog.scheduledOpenAt < nowLocal
      ? "计划开门时间已到，等待轮值看门人确认。"
      : "点进值班记录，可以看看今天谁在客厅。"),
    currentLog: payload,
  };
...
async function currentRoomStatus(req = null) {
  ...
  return roomStatusFromLogs([...(todayResult.data || []), ...overnightLogs], nowLocal);
}
...
async function invalidatePublicActivityCaches(activityId = "") {
...
async function adminDashboardPayload(user) {
  ...
  const [byStatus, usersTotal, rolesTotal, modulesTotal, templatesTotal, friendsTotal, feedbackReviewTotal, roomLogReviewTotal, pending, feedbackPreview, roomLogPreview] = await Promise.all([
  ...
  const pendingTotal = Number(pending.total || 0) + Number(feedbackReviewTotal || 0) + Number(roomLogReviewTotal || 0);
  return {
    ...
    feedbacks: { pendingReview: feedbackReviewTotal, roomLogReview: roomLogReviewTotal },
    pending: {
      ...pending,
      total: pendingTotal,
      activityTotal: pending.total || 0,
      feedbackTotal: feedbackReviewTotal,
      roomLogTotal: roomLogReviewTotal,
      feedbacks,
      roomLogs,
    },
  };
...
async function safetyHealthPayload() {
  const [settings, usageToday, usageWeek, pendingAnalysis, queuedJobs, adminReview, hiddenReview, feedbackReview, roomLogReview, reportWarnings, highRiskActivities, recentReports] = await Promise.all([
    getAiSettings(store),
...
function createApp(options = {}) {
  ...
  app.get("/api/room-logs", asyncRoute(async (req, res) => {
    ...
    const { data, pageInfo } = await store.query("roomDutyLogs", {
    ...
    const context = await identityNetworkContextForRequest(req);
    res.json({
      roomLogs: data.map((log) => publicRoomLog(log, {
        includePrivate: includeAll || canManageRoomLog(log, user, req, context),
        canManage: canManageRoomLog(log, user, req, context),
      })),
      pageInfo,
    });
  }));
  ...
  app.get("/api/room-logs/:id", asyncRoute(async (req, res) => {
    ...
    req.currentUser = user;
    const context = await identityNetworkContextForRequest(req);
    if (!log || (!canManageRoomLog(log, user, req, context) && log.isHidden)) {
      res.status(404).json({ error: "找不到这条开门记录" });
      return;
    }
    res.json({
      roomLog: publicRoomLog(log, {
        includePrivate: canManageRoomLog(log, user, req, context) || userCan(user, "feedbacks", "view"),
        canManage: canManageRoomLog(log, user, req, context),
      }),
    });
  }));
  ...
  app.get("/api/my/room-logs", asyncRoute(async (req, res) => {
    ...
    res.json({
      roomLogs: data.map((log) => publicRoomLog(log, { includePrivate: true, canManage: true })),
      pageInfo,
    });
  }));
  ...
  const updateActivityHandler = asyncRoute(async (req, res) => {
    ...
    await invalidatePublicActivityCaches(finalUpdated.id);
    if (!asDraft) {
