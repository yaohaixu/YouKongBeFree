...
const { analyzeFeedback, getAiSettings, publicAiSettings, saveAiSettings, testAiConnection } = require("./ai-analysis/service");
...
async function adminDashboardPayload(user) {
  const [byStatus, usersTotal, modulesTotal, templatesTotal, friendsTotal, feedbackReviewTotal, pending, feedbackPreview] = await Promise.all([
...
async function toActivityPayload(activity, options = {}) {
  ...
  const [module, owner, collaborator, friend, activityRegistrations, interestMap] = await Promise.all([
...
async function toActivityListPayload(activities, options = {}) {
  ...
  const [moduleMap, userMap, friendMap, interestMap] = await Promise.all([
...
function createApp(options = {}) {
  ...
  app.post("/api/ai/prompts/:id/activate", requireAdmin, asyncRoute(async (req, res) => {
    ...
    const settings = await saveAiSettings(store, settingsPatch);
  ...
  app.post("/api/living-room-friends", requireAdmin, upload.single("logo"), asyncRoute(async (req, res) => {
  ...
  app.put("/api/living-room-friends/:id", requireAdmin, upload.single("logo"), asyncRoute(async (req, res) => {
  ...
  app.delete("/api/living-room-friends/:id", requireAdmin, asyncRoute(async (req, res) => {
  ...
  app.get("/api/feedbacks/export", requireAdmin, asyncRoute(async (req, res) => {
    const keyword = cleanText(req.query.q).toLowerCase();
    const { data } = await store.query("activityFeedbacks", {
      page: 1,
      pageSize: 1000,
  ...
  app.get("/api/feedbacks", requireAdmin, asyncRoute(async (req, res) => {
    const keyword = cleanText(req.query.q).toLowerCase();
    const { data, pageInfo } = await store.query("activityFeedbacks", {
      ...pageQueryOptions(req.query),
      filters: feedbackFilters(req.query),
  ...
  app.post("/api/feedbacks/:id/review", requireAdmin, asyncRoute(async (req, res) => {
