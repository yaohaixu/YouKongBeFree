...
function runRule(rule = {}, context = {}) {
  ...
  if (rule.type === "sensitive_terms") {
    ...
    const extra = Math.min(
      Number(params.maxExtraWeight || 0),
      Math.max(0, hits.length - 1) * Number(params.perHitWeight || 0)
    );
    return hits.length ? [finding(rule, weight + extra, `命中 ${hits.length} 个敏感词`, hits)] : [];
  }
