...
function manageTokenMaxAgeMs() {
  const days = Math.max(1, Math.min(Number(process.env.MANAGE_TOKEN_MAX_AGE_DAYS || 180), 365));
  return days * 24 * 60 * 60 * 1000;
}

function manageTokenExpiresAt(now = new Date()) {
  return new Date(now.getTime() + manageTokenMaxAgeMs()).toISOString();
}

function manageTokenIdentityPatch(identity = {}) {
  return {
    manageTokenClientIdHash: identity.clientIdHash || "",
    manageTokenServerIdHash: identity.serverIdHash || "",
    manageTokenFingerprintHash: identity.fingerprintHash || "",
  };
}

function issueManageToken(activityId, identity = {}, now = new Date()) {
  const token = makeManageToken();
  return {
    token,
    manageTokenHash: hashManageToken(activityId, token),
    manageTokenCreatedAt: now.toISOString(),
    manageTokenExpiresAt: manageTokenExpiresAt(now),
    ...manageTokenIdentityPatch(identity),
  };
}
...
function getManageToken(req) {
  const token = req.get("X-YK-Manage-Token") || req.body?.manageToken || "";
  if (token) return String(token).trim();
  if (process.env.ALLOW_MANAGE_TOKEN_QUERY === "true") {
    return String(req.query.manageToken || "").trim();
  }
  return "";
}

function manageTokenExpired(activity = {}) {
  const explicit = activity.manageTokenExpiresAt ? new Date(activity.manageTokenExpiresAt).getTime() : 0;
  if (explicit) return explicit <= Date.now();
  const createdAt = activity.manageTokenCreatedAt || activity.createdAt || "";
  const legacyCreated = createdAt ? new Date(createdAt).getTime() : 0;
  if (!legacyCreated) return false;
  return legacyCreated + manageTokenMaxAgeMs() <= Date.now();
}

function manageTokenMatchesIdentity(activity = {}, identity = {}) {
  const stored = [
    activity.manageTokenClientIdHash,
    activity.manageTokenServerIdHash,
    activity.manageTokenFingerprintHash,
  ].filter(Boolean);
  if (!stored.length) return true;
  const current = new Set([
    identity.clientIdHash,
    identity.serverIdHash,
    identity.fingerprintHash,
  ].filter(Boolean));
  return stored.some((item) => current.has(item));
}

function verifyManageToken(activity, token = "", identity = {}) {
  if (!activity?.manageTokenHash || !token) return false;
  if (activity.manageTokenRevokedAt || manageTokenExpired(activity)) return false;
  if (!manageTokenMatchesIdentity(activity, identity)) return false;
...
async function storeAnalysisReport(store, activityId, analysis, context = {}) {
  ...
  const report = {
    ...
    aiMeta: {
      ...
      forced: Boolean(context.forceAi),
      promptVersion: analysis.aiResult?.prompt?.version || "",
      promptName: analysis.aiResult?.prompt?.name || "",
      profileId: analysis.aiResult?.profile?.id || "",
      profileName: analysis.aiResult?.profile?.name || "",
      attempts: analysis.aiResult?.attempts || [],
...
async function submitCommunityReport(store, req, activity, reason, detail = "") {
  ...
  const identityNetworkId = String(req?.ykIdentityContext?.network?.id || "").trim();
  const identityIds = Array.from(new Set([
    ...(Array.isArray(req?.ykIdentityContext?.deviceIds) ? req.ykIdentityContext.deviceIds : []),
    context.identity.id,
  ].filter(Boolean)));
  ...
  const existing = (identityNetworkId
    ? await store.findByFilters("communityReports", [
      { field: "activityId", op: "eq", value: activity.id },
      { field: "identityNetworkId", op: "eq", value: identityNetworkId },
    ])
    : null) || await store.findByFilters("communityReports", [
      { field: "activityId", op: "eq", value: activity.id },
      { field: "identityId", op: identityIds.length > 1 ? "in" : "eq", value: identityIds.length > 1 ? identityIds : context.identity.id },
    ]);
  if (existing) {
