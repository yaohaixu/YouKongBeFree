...
function makeTrustProfile(identity, config = {}) {
  ...
  const score = clamp(initial, config.min, config.max);
  return {
    id: identity.id,
    communityTrust: score,
    communityId: String(identity.id || "").replace(/^anon_/, "").slice(0, 4).toUpperCase() + "...",
    communityLevel: score >= 70 ? "trusted_partner" : score < 45 ? "watch" : "normal",
    status: score < 25 ? "restricted" : score < 45 ? "watch" : "normal",
    badges: [],
